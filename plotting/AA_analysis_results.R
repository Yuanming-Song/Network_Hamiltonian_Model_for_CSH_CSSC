...
# Load AA ERGM results
load_aa_results <- function() {
  aa_file <- file.path(results_dir, "csh_cssc_1node_fit.Rdata")
  
  if (!file.exists(aa_file)) {
    stop("AA results file not found: ", aa_file)
  }
  
  # Load the data
  env <- new.env()
  load(aa_file, envir = env)
...
# Create LaTeX table for AA ERGM results
create_aa_latex_table <- function(best_fit, save_file = FALSE) {
  ...
  latex_lines <- c(
    "\\begin{table}[ht]",
    "\\centering",
    "\\caption{Atomistic ERGM Parameter Estimates (5 Systems)}",
    "\\label{tab:aa_ergm}",
    "\\begin{tabular}{lrrrr}",
    "\\hline",
  ...
  if (save_file) {
    ...
    output_file <- file.path(output_dir, "AA_ERGM_table.tex")
    
    # Add LaTeX-ready header comments for coding agent
    final_latex <- c(
      "% AA ERGM Parameter Table (5 Systems)",
      "% To include in LaTeX document, use: \\input{AA_ERGM_table.tex}",
      "% This table shows Atomistic ERGM parameter estimates with 5-system composition distribution",
      "",
      latex_lines
    )
...
cat("Loading AA ERGM results...\n")
results <- load_aa_results()
...
latex_table <- create_aa_latex_table(results$best_fit, save_file = save_plot)
...
# Create system labels mapping (5 systems)
system_labels <- c(
  "0% CSSC (CSH 32)", 
  "17% CSSC (CSSC 24)", 
  "33% CSSC (CSH 16, CSSC 8)", 
  "50% CSSC (CSH 12, CSSC 12)", 
  "83% CSSC (CSH 24, CSSC 4)"
)

# Function to create GOF subplot for one system (5-system version)
create_aa_gof_subplot <- function(obs_data, mean_data, q025_data, q975_data, 
  ...
  p <- ggplot(plot_data, aes(x = x)) +
    ...
    labs(title = paste(stat_name, "-", system_labels[system_type]), 
...
# Function to create legend plot (same as CG version)
...
for (sys in 1:5) {
  ...
  # Remove individual titles for cleaner look
  if (sys != 1) p <- p + labs(title = "")
  degree_plots[[sys]] <- p
}
...
degree_combined <- grid.arrange(
  ...
  top = "Degree Distribution - Goodness of Fit (Atomistic)",
  left = "Vertices",
  bottom = "Degree"
)

# 2. ESP DISTRIBUTION PLOTS (5 systems in 3 columns)
...
esp_combined <- grid.arrange(
  ...
  ncol = 3, nrow = 2,
  layout_matrix = rbind(c(1, 2, 3), c(4, 5, 6)),
  top = "ESP Distribution - Goodness of Fit (Atomistic)",
  left = "Vertices", 
  bottom = "Edgewise Shared Partners"
)

# 3. COMPONENT STATISTICS PLOTS (5 systems in 3 columns)
...
comp_combined <- grid.arrange(
  ...
  ncol = 3, nrow = 2,
  layout_matrix = rbind(c(1, 2, 3), c(4, 5, 6)),
  top = "Component Statistics - Goodness of Fit (Atomistic)",
  left = "Count",
...
if (save_plots) {
  ...
  png(file.path(output_dir, "AA_degree_GOF.png"), width = 2400, height = 1200, res = 300)
  grid.arrange(
    grobs = c(degree_plots, list(legend_plot)), 
    ncol = 3, nrow = 2,
    layout_matrix = rbind(c(1, 2, 3), c(4, 5, 6)),
  ...
  png(file.path(output_dir, "AA_esp_GOF.png"), width = 2400, height = 1200, res = 300)
  grid.arrange(
    grobs = c(esp_plots, list(legend_plot)), 
    ncol = 3, nrow = 2,
    layout_matrix = rbind(c(1, 2, 3), c(4, 5, 6)),
  ...
  png(file.path(output_dir, "AA_comp_GOF.png"), width = 2400, height = 1200, res = 300)
  grid.arrange(
    grobs = c(comp_plots, list(legend_plot)), 
    ncol = 3, nrow = 2,
    layout_matrix = rbind(c(1, 2, 3), c(4, 5, 6)),
  ...
  cat("AA GOF plots saved to:", output_dir, "\n")
}

cat("AA analysis complete!\n")
